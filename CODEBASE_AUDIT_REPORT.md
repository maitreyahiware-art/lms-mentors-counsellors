...
**Project:** Balance Nutrition — Counselor LMS Platform  
...
7. [Admin Portal — Mentor Logs Accuracy](#7-admin-portal--mentor-logs-accuracy)
...
| Database | Supabase (PostgreSQL) | Tables: `profiles`, `mentor_progress`, `assessment_logs`, `summary_audits`, `simulation_logs`, `mentor_activity_logs` |
...
- `mentor_progress` — topic completion per user
...
- `mentor_activity_logs` — general activity log
...
|---|-------|----------|------|---------|
| 3.1 | **No server-side role verification on admin API routes.** The `create-user`, `force-reset`, and `seed-mentors` API endpoints do NOT verify that the calling user has admin privileges. Any authenticated user can call `POST /api/admin/create-user` and create accounts. | **CRITICAL** | `src/app/api/admin/create-user/route.ts` | All |
...
| 3.4 | **Inconsistent default role.** On initial auth check: `role = session.user.user_metadata?.role || 'interviewee'` (L46), but on `SIGNED_IN` event: `role = session.user.user_metadata?.role || 'mentor'` (L81). This inconsistency can cause role mis-assignment. | **MEDIUM** | `src/app/layout.tsx` | L46 vs L81 |
...
| 3.6 | **Signup is open to anyone.** The interviewee pathway exposes a "Sign Up" tab. Anyone can create an account, and the role is set client-side. There is no invitation-only or admin-gated account creation flow for mentors. | **MEDIUM** | `src/app/login/page.tsx` | L51-82 |
...
- Completed state is correctly persisted to Supabase `mentor_progress` table and re-fetched on page load.
...
### ✅ What's Working
- `SummaryGrader.tsx` allows mentors to write summaries, which are graded by AI.
- Results (score + feedback) are saved to `summary_audits` table.
- The admin dashboard fetches and displays peer audit counts per mentor.
...
## 7. Admin Portal — Mentor Logs Accuracy

### ✅ What's Working
- The "Mentor Logs" tab correctly fetches from `profiles`, `assessment_logs`, `mentor_activity_logs`, and `summary_audits`.
- Mentor cards display computed progress, average score, audit counts, activity counts, and last active date.
...
| # | Issue | Severity | File | Line(s) |
|---|-------|----------|------|---------|
| 7.1 | **Progress calculation is based on `assessment_logs`, NOT `mentor_progress`.** The admin dashboard calculates progress from unique `topic_code` values in `assessment_logs` (L173: `const uniqueCompletedTopics = new Set(mentorAssessments.map(...))`). But actual topic completion is tracked in `mentor_progress`. A mentor could complete all topics (via the completion button) but show 0% progress on admin if they haven't taken quizzes. This is a **fundamental data source mismatch.** | **CRITICAL** | `admin/page.tsx` | L173-177 |
| 7.2 | **User dashboard uses `mentor_progress`, admin uses `assessment_logs` for the same metric.** On the user-facing homepage (`page.tsx` L47-52), progress comes from `mentor_progress`. On admin (`admin/page.tsx` L173), it comes from `assessment_logs`. These produce **completely different numbers.** | **CRITICAL** | `page.tsx` L47 vs `admin/page.tsx` L173 |
| 7.3 | **"Joined" date shows last activity, not creation date.** The mentor card shows `mentor.stats?.lastActive || mentor.created_at` as the "Joined" date label (L556), which is confusing. | **LOW** | `admin/page.tsx` | L556 |
| 7.4 | **No pagination or search for mentors.** All mentors are loaded at once with no filtering. This will not scale. | **LOW** | `admin/page.tsx` | L537-621 |
...
- The `force-reset` and `seed-mentors` endpoints can batch-create/reset mentor accounts.
...
| 8.4 | **`sync-profile` endpoint always sets role to `'mentor'`.** If an admin account calls this, their role would be overwritten to `'mentor'`. | **MEDIUM** | `auth/sync-profile/route.ts` | L20 |
...
- The recent `.upsert()` fix for `mentor_progress` resolves the 409 conflict issue.
...
| 10.4 | **`mentor_activity_logs` table may not exist.** The `logActivity` function fails silently with a `console.warn` if the table doesn't exist (L31). This means activity tracking might be completely non-functional without anyone knowing. | **MEDIUM** | `lib/activity.ts` | L28-31 |
...
| 11.2 | **Activity logs don't include the user's name.** The admin feed must resolve user IDs to names by joining with the `mentors` array client-side. | **LOW** | `admin/page.tsx` | L639-657 |
...
| 12.4 | **All mentor passwords are the same** (`515148`), set by `force-reset` and `seed-mentors`. | **HIGH** | `api/admin/force-reset/route.ts` L44 |
...
| **Sidebar Role-Based UI** | ✅ Working | Different nav items shown for admin vs. mentor |
...
| **Mentor Profile Cards** | ✅ Partially Working | Displays mentor information, but progress data source is wrong |
...
4. **Fix admin progress calculation.** Change admin dashboard to use `mentor_progress` (not `assessment_logs`) for the training progress metric:
   ```ts
   // Fetch mentor_progress per user
   const { data: progress } = await supabase.from('mentor_progress').select('user_id, topic_code');
...
13. **Add pagination and search** to the Mentor Logs tab.
14. **Create a proper invitation system** instead of open signup for mentors.
15. **Add error toasts** instead of `alert()` for user-facing messages.
16. **Verify `mentor_activity_logs` table exists** in DB schema, or create it.
