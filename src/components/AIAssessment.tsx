...
export default function AIAssessment({ topicTitle, topicContent, topicCode, onComplete }: AIAssessmentProps) {
    ...
    const submitAudit = async (finalAnswers: string[]) => {
        // Assessment Complete
        const finalScore = finalAnswers.reduce((acc, curr, idx) => {
        ...
        if (session && questions) {
            ...
            await supabase.from('assessment_logs').insert([{
                ...
                raw_data: { questions, answers: finalAnswers, time_spent: 600 - timeLeft }
            }]);
        }
            ...
            <AnimatePresence mode="wait">
                ...
                {showResult && (
                    ...
                    >
                        ...
                        <div className="inline-flex items-center gap-3 px-8 py-3 bg-[#0E5858] text-white rounded-[1.2rem] text-xs font-bold shadow-2xl opacity-90 cursor-default">
